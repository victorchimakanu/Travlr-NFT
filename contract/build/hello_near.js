...
let TravlrNFT = (_dec = NearBindgen({}), _dec2 = initialize({}), _dec3 = call({}), _dec4 = view({}), _dec(_class = (_class2 = class TravlrNFT {
  constructor() {
    this.owner_id = "";
  ...
  mint({
    ...
    assert(predecessorAccountId() == this.owner_id, "Unauthorized transaction"); // checks to make sure its a valid transaction
...
}, (_applyDecoratedDescriptor(_class2.prototype, "init", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "init"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "mint", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "mint"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_token_by_id", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "get_token_by_id"), _class2.prototype)), _class2)) || _class);
function get_token_by_id() {
  let _state = TravlrNFT._getState();
  if (!_state && TravlrNFT._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  let _contract = TravlrNFT._create();
  if (_state) {
    TravlrNFT._reconstruct(_contract, _state);
  }
  let _args = TravlrNFT._getArgs();
  let _result = _contract.get_token_by_id(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TravlrNFT._serialize(_result));
}
function mint() {
  let _state = TravlrNFT._getState();
  if (!_state && TravlrNFT._requireInit()) {
  ...
  TravlrNFT._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TravlrNFT._serialize(_result));
}
function init() {
  let _state = TravlrNFT._getState();
...
export { get_token_by_id, init, mint };
